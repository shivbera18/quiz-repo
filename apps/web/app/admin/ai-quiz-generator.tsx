...
export default function AIQuizGenerator({ onQuizCreated, onClose }: AIQuizGeneratorProps) {
  ...
  const generateQuiz = async () => {
    ...
    try {
      // catalog-svc generates a quiz asynchronously now (a Kafka worker builds
      // it section-by-section, persisting partial progress as it goes -- the
      // actual fix for the old bug where one failed section discarded every
      // question already generated). This POST just queues the job; we poll
      // for its result below instead of getting the quiz back immediately.
      ...
      if (!response.ok) {
        ...
        setError(`❌ Failed to queue quiz generation: ${errorData.message || response.status}`)
      ...
      // Poll every 3s for up to 5 minutes -- generation is section-by-section
      // Gemini calls, so this can take a while for a large question count.
      const maxAttempts = 100
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        await new Promise((resolve) => setTimeout(resolve, 3000))

        const jobRes = await fetch(`/api/ai/generate-quiz/${jobId}`, {
        ...
        if (job.status === "pending" || job.status === "in_progress") {
          const generatedSoFar = job.partialQuestions ? Object.keys(job.partialQuestions).length : 0
          setGenerationStatus(`Generating questions... (${generatedSoFar}/${formData.sections.length} sections done)`)
          continue
        }

        if (job.status === "failed") {
          setError(`❌ AI Generation Failed: ${JSON.stringify(job.failures)}`)
          setIsGenerating(false)
          return
        }

        // "succeeded" or "partial" -- a quiz was created either way (partial
        // lands isActive:false for admin review, per catalog-svc's worker).
        if (!job.resultQuizId) {
        ...
        const quizRes = await fetch(`/api/admin/quizzes/${job.resultQuizId}`, {
          headers: { 'Authorization': `Bearer ${user?.token || ""}` },
        })
        if (!quizRes.ok) {
          setError("❌ Quiz was generated but could not be loaded. Check the admin quiz list.")
          setIsGenerating(false)
          return
        }
        ...
        setSuccess(
          job.status === "partial"
            ? `⚠️ AI Quiz "${quiz.title}" created as a draft (some sections failed -- review before activating).`
            : `✅ AI Quiz "${quiz.title}" created successfully with ${quiz.questions.length} questions!`
        )
        onQuizCreated(quiz)
      ...
      setError("❌ Timed out waiting for AI quiz generation. It may still finish in the background -- check the admin quiz list shortly.")
      setIsGenerating(false)
    } catch (err) {
