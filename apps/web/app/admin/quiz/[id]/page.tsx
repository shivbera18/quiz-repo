...
interface Quiz {
  ...
  createdAt: string
  createdBy: string
  // Optimistic-concurrency token -- catalog-svc's PATCH requires the version
...
export default function QuizManagementPage(props: { params: Promise<{ id: string }> }) {
  ...
  const saveQuiz = async (updatedQuiz: Quiz) => {
    try {
      ...
      if (res.status === 409) {
        throw new Error("This quiz was modified by someone else. Reload and try again.")
      }
      ...
      setError(err instanceof Error ? err.message : "Failed to save quiz to database")
    }
  }
            ...
            {quiz.questions.length === 0 ? (
              ...
              <div className="space-y-4">
                {quiz.questions.map((question, index) => (
                  <div key={question.id} className="border-2 border-black dark:border-white/65 rounded-lg p-4 shadow-[2px_2px_0px_0px_#000] dark:shadow-[2px_2px_0px_0px_rgba(255,255,255,0.65)]">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div className="flex gap-2 ml-4">
                        <Button variant="ghost" size="icon" onClick={() => handleEditQuestion(question)} className="border-2 border-black dark:border-white/65">
                          <Edit className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => handleDeleteQuestion(question.id)} className="border-2 border-black dark:border-white/65 text-red-500">
