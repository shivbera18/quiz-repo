...
// FOR UPDATE SKIP LOCKED claim -> Kafka send -> mark-published, all inside one
// transaction. markPublished is a closure bound to THIS transaction's client;
// a separate pool-client method would self-block on our own row locks every
// batch. See packages/kafka-kit/src/outbox.ts for the full contract.
