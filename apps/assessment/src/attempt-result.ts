// Adapted from the monolith's lib/attempt-result.ts. Same output shape (so
// apps/web's results page needs a URL change, not a rewrite) with one
// difference: quizTitle comes from the AttemptSnapshot this service already
// holds (snapshot.quizTitle), not a separate cross-service lookup back to
// catalog-svc for every result view.
//
// Unit note, preserved intentionally rather than "fixed": the results page's
// own math treats the top-level `timeSpent` as SECONDS but each per-question
// `timeSpent` as MILLISECONDS. That inconsistency predates this port; this
// formatter matches it rather than introducing a third convention the page
// wasn't built to read.

import type { Attempt, AttemptAnswer } from "./generated/prisma/index.js"
